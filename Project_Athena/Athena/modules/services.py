...
@listen_to('What services do we have|List services', re.IGNORECASE)
...
@respond_to('What services do we have|List services', re.IGNORECASE)
...
def service_check(message, service):
    ...
    if query is None:
        ...
        if staged is not None:
            load_time = check_load_time(staged[0].check_url)
            message.reply(std_reply.format(query[0].service_name, load_time))
        else:
